...
' | tee -a /etc/pam.d/sshd | tee -a /etc/pam.d/cockpit | tee -a $LOGFILE | tee -a $CONFIG_LOG
gauth_command=/usr/local/bin/gauth_enable_totp
touch $gauth_command
chmod -v 555 /usr/local/bin/** | tee -a $LOGFILE | tee -a $CONFIG_LOG
echo '
#!/bin/zsh
...
' | tee -a $gauth_command | tee -a $LOGFILE | tee -a $CONFIG_LOG
echo "
#############################################################
#############################################################
#############################################################
...
#############################################################
" | tee -a $LOGFILE
speedtest --accept-license --accept-gdpr | tee -a $LOGFILE
